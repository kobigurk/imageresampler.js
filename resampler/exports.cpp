...
extern "C" int resample(char * pFilter, unsigned char* pSrc_image, int src_width, int src_height, int n, unsigned char* pDst_image, int dst_width, int dst_height)
{
   ...
   for (int src_y = 0; src_y < src_height; src_y++)
   {
      ...
      for ( ; ; )
      {
         ...
         for (comp_index = 0; comp_index < n; comp_index++)
         {
            ...
            unsigned char* pDst = &pDst_image[dst_y*dst_pitch+comp_index];
